...
from DiT4DiT.dataloader.gr00t_lerobot.transform.video import (
    VideoColorJitter,
    VideoCrop,
    VideoResize,
    VideoToNumpy,
...
class UnitreeG1AlohaOnlyArmsDataConfig(UnitreeG1DataConfig):
    video_keys = ["video.ego_view"]
    state_keys = [
        "state.left_arm",
        "state.right_arm",
...
class G1BallPickerDataConfig(BaseDataConfig):
    """DataConfig for the lerobot_0507 G1 ball-picker dataset.

    Modality layout (from modality.json):
      state/action: action_delta_eef(0:12), delta_height(12:13),
                    teleop_navigate(13:16), hand_status(16:18)  -> total 18
      video:        left  (original_key: observation.images.left)
      annotation:   human.task_description
    """
    video_keys = ["video.left"]
    state_keys = [
        "state.action_delta_eef",
        "state.delta_height",
        "state.teleop_navigate",
        "state.hand_status",
    ]
    action_keys = [
        "action.action_delta_eef",
        "action.delta_height",
        "action.teleop_navigate",
        "action.hand_status",
    ]
    language_keys = ["annotation.human.task_description"]
    observation_indices = [0]
    ...
    def transform(self) -> ModalityTransform:
        transforms = [
            ...
            StateActionTransform(
                apply_to=self.action_keys,
                normalization_modes={
                    "action.action_delta_eef": "min_max",
                    "action.delta_height": "min_max",
                    "action.teleop_navigate": "min_max",
                    "action.hand_status": "binary",
                },
            ),
        ]
        return ComposedModalityTransform(transforms=transforms)
...
class G1PingpangqiuDataConfig(BaseDataConfig):
    """DataConfig for lerobot_0510 — G1 pingpangqiu (table-tennis ball picking).

    Action space (15 dims):
      torso(3) + right_arm(7) + right_hand(1) + teleop_navigate(3) + delta_height(1)
    State space (27 dims, 54 after SinCos):
      legs_torso(15) + right_arm(7) + right_hand(1) + teleop_navigate(3) + delta_height(1)
    """
    video_keys = ["video.left"]
    state_keys = [
        "state.legs_torso",
...
class G1WholebodyTeleopDataConfig(BaseDataConfig):
    """DataConfig for G1 wholebody teleop (e.g. OpenOven / OpenTrashCan).
    ...
    Video:      video.rs_view (egocentric)
    """
    ...
    language_keys = ["annotation.human.task_description"]
    observation_indices = [0]
    action_indices = list(range(16))  # action chunk=16
...
ROBOT_TYPE_CONFIG_MAP = {
    ...
    "g1_ball_picker": G1BallPickerDataConfig(),
    "G1_PINGPANGQIU": G1PingpangqiuDataConfig(),
