...
from DiT4DiT.dataloader.gr00t_lerobot.video import get_all_frames, get_frames_by_timestamps

from DiT4DiT.dataloader.gr00t_lerobot.embodiment_tags import EmbodimentTag
from DiT4DiT.dataloader.gr00t_lerobot.schema import (
    DatasetMetadata,
...
class LeRobotSingleDataset(Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Allow config-time overrides (useful for future-frame supervision / multi-frame conditioning)
        # Example: cfg.datasets.vla_data.video_delta_indices: [0, 1]
        if self.data_cfg is not None:
            try:
                video_delta = self.data_cfg.get("video_delta_indices", None)
            except Exception:
                video_delta = None
        ...
        for attempt in range(max_retries):
            try:
                ...
                def _to_img_tensor(image_any) -> torch.Tensor:
                    ...
                    if torch.is_tensor(image_any):
                        t = image_any.detach().cpu()
                        if t.ndim == 3 and t.shape[0] in (1, 3, 4):  # CHW
                            ...
                            if t.shape[-1] == 1:
                                t = t.repeat(1, 1, 3)
                            img_t = t.permute(2, 0, 1).contiguous().float()
                        else:
                ...
                # Pixel-space temporal downsampling (like FastWAM's action_video_freq_ratio)
                action_video_freq_ratio = int(self.data_cfg.get("action_video_freq_ratio", 1)) if self.data_cfg else 1
                ...
                for video_key in dataset.modality_keys["video"]:
                    ...
                    # Normalize different containers to indexable frames:
                    # - list/tuple: [T] of HWC arrays
                    # - np.ndarray: (T,H,W,C) or (H,W,C)
                    # - torch.Tensor: (T,H,W,C) or (T,C,H,W) or single frame (H,W,C)/(C,H,W)
                    if isinstance(frames, (list, tuple)):
                        if len(frames) == 0:
                            continue
                        get_frame = lambda idx: frames[idx]  # noqa: E731
                        ...
                        if frames.ndim == 4:
                            ...
                            num_frames = 1
                            get_frame = lambda idx: frames  # noqa: E731
                        else:
                            continue
                        ...
                        if t.ndim == 4:
                            ...
                            get_frame = lambda idx: t[idx]  # noqa: E731
                        elif t.ndim == 3:
                            num_frames = 1
                            get_frame = lambda idx: t  # noqa: E731
                        else:
                            continue
                    else:
                        continue
                ...
                if n_state_dims > max_state_dim:
                    ...
                    state = np.pad(state, ((0, 0), (0, max_state_dim - n_state_dims)), "constant")
