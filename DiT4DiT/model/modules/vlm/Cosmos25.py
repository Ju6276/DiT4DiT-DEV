...
class Cosmos25FeatureExtractor(nn.Module):
    ...
    def device(self) -> torch.device:
        # In memory-constrained inference we may keep the very large text encoder
        # on CPU while the video transformer/vae stay on GPU.
    ...
    def _get_prompt_embeds(
        ...
        *,
        ...
        if self.text_encoder is None:
            raise RuntimeError("text_encoder is unavailable; pass precomputed `prompt_embeds` instead.")
        ...
        input_ids_batch = torch.stack(input_ids_batch, dim=0).to(text_encoder_device)
        outputs = self.text_encoder(input_ids_batch, output_hidden_states=True)
        hidden_states = outputs.hidden_states

        normalized_hidden_states = []
        for layer_idx in range(1, len(hidden_states)):
            hs = hidden_states[layer_idx].to(device=device)
    ...
    def encode_prompt(
        ...
        *,
        ...
        if dtype is None:
            dtype = prompt_embeds.dtype if prompt_embeds is not None else self.text_encoder.dtype
