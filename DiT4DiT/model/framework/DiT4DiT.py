    ...
    def predict_action(
        ...
        **kwargs: str,
        ...
        if "prompt_embeds" in examples[0]:
            prompt_embed_list = [example["prompt_embeds"] for example in examples]
            if torch.is_tensor(prompt_embed_list[0]):
                if prompt_embed_list[0].ndim == 3 and len(prompt_embed_list) == 1:
                    prompt_embeds = prompt_embed_list[0]
                else:
                    prompt_embeds = torch.stack(
                        [pe.squeeze(0) if pe.ndim == 3 else pe for pe in prompt_embed_list],
                        dim=0,
                    )
            else:
                prompt_embeds = torch.tensor(np.array(prompt_embed_list))
        ...
        if prompt_embeds is not None:
            backbone_inputs["prompt_embeds"] = prompt_embeds
        with torch.autocast("cuda", dtype=torch.bfloat16):
            backbone_outputs = self.backbone_interface(
                **backbone_inputs,
                output_attentions=False,
