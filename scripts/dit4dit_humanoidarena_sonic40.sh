...
CONFIG_PATH="${REPO_ROOT}/DiT4DiT/config/humanoidarena/dit4dit_sonic40.yaml"
...
BASE_MODEL="${BASE_MODEL:?Set BASE_MODEL to the local diffusers-format Cosmos-Predict2.5-2B directory}"
...
RUN_ID="humanoidarena-sonic-${TASK}-dit4dit-canonical40-h30-100k"
echo "Training DiT4DiT task=${TASK} processes=${NUM_PROCESSES} global_batch=${GLOBAL_BATCH_SIZE} per_device_batch=${PER_DEVICE_BATCH_SIZE} accumulation=${GRADIENT_ACCUMULATION_STEPS} steps=${MAX_STEPS}"
