"""Strict HumanoidArena SONIC40 HTTP and normalization contract."""
...
def normalize_state(state: np.ndarray, stats: dict[str, Any]) -> np.ndarray:
    ...
    if low.shape != (STATE_DIM,) or high.shape != (STATE_DIM,):
        raise ValueError("checkpoint state statistics must be 64D")
    mask = high != low
...
def denormalize_action(action: Any, stats: dict[str, Any]) -> np.ndarray:
    ...
    if normalized.shape != (ACTION_HORIZON, ACTION_DIM):
        raise ValueError(
            f"DiT4DiT action must be [{ACTION_HORIZON},{ACTION_DIM}], got {normalized.shape}"
        )
    low = np.asarray(stats["min"], dtype=np.float32)
    high = np.asarray(stats["max"], dtype=np.float32)
    if low.shape != (ACTION_DIM,) or high.shape != (ACTION_DIM,):
        raise ValueError("checkpoint action statistics must be 40D")
    result = 0.5 * (np.clip(normalized, -1, 1) + 1) * (high - low) + low
    result[:, 38:40] = (normalized[:, 38:40] > 0.5).astype(np.float32)
    if not np.isfinite(result).all():
        raise ValueError("DiT4DiT action contains non-finite values")
    return result
